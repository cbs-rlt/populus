...
DrawingPane drawingPanel;
   ...
   public StageStructuredPane(int type){
      ...
      addStage.addActionListener(new java.awt.event.ActionListener() {
         public void actionPerformed(ActionEvent e) {
            addStage_actionPerformed(e);
         }
      });
	...
	//Color background = Color.green;
      ...
      */
      ...
      Transition setUpTransition(int stageStart, int stageStop){
         ...
         for(int i=0; i<trans.size(); i++){
            c = ((Transition)trans.get(i)).getConnectionIndicies();
            if(c[0] == stageStop && c[1] == stageStart){
               t.setStraight(false);
               ((Transition)trans.get(i)).setStraight(false);
            }
      ...
      public void paint(Graphics g){
         ...
         for(int i=0; i<trans.size(); i++){
            ((Transition)trans.get(i)).setTimeIndex(timeIndex);
            ((Transition)trans.get(i)).paint(g);
         }
         for(int i=0; i<stages.size(); i++){
            Stage s = (Stage)stages.get(i);
            s.setIndexShift(indexShift);
            s.paint(g);
         }
         for(int i=0; i<trans.size(); i++)
            ((Transition)trans.get(i)).paintLabel(g);
         for(int i=0; i<stages.size(); i++)
            ((Stage)stages.get(i)).paintLabel(g);
      ...
      public StageShape getSelection(Point p){
         for(int i=0; i<trans.size(); i++)
            if(((Transition)trans.get(i)).getLabel().contains(p))
               return ((Transition)trans.get(i)).getLabel();
         for(int i=0; i<stages.size(); i++)
            if(((Stage)stages.get(i)).getLabel().contains(p))
               return ((Stage)stages.get(i)).getLabel();
         for(int i=0; i<stages.size(); i++)
            if(((Stage)stages.get(i)).contains(p))
               return (StageShape)stages.get(i);
         for(int i=0; i<trans.size(); i++)
            if(((Transition)trans.get(i)).contains(p))
               return (StageShape)trans.get(i);
         return null;
      }
      ...
      public void mouseClicked(MouseEvent e) {
         ...
         if(stageTB != null && stageTB.isSelected()){
            if(!queue.isEmpty()){
               ...
               Stage s = (Stage)queue.firstElement();
      ...
      public void mouseMoved(MouseEvent e) {
         ...
         for(int i=0; i<stages.size(); i++){
            ((Stage)stages.get(i)).setActive(false);
            ((Stage)stages.get(i)).l.setActive(false);
         }
         for(int i=0; i<trans.size(); i++){
            ((Transition)trans.get(i)).setActive(false);
            ((Transition)trans.get(i)).l.setActive(false);
         }
      ...
      public void setStageDefaultPositions(){
         if(type == StageStructuredPane.kASPG){
            ...
            for(int i=0; i<stages.size(); i++){
               ((Stage)stages.get(i)).setPosition(new Point((int)(cushion+i*xdistance),(int)(cushion+i*ydistance)),r);
            }
         }
      }
      ...
      void this_componentResized(ComponentEvent e) {
         ...
         default positions*/
         for(int i=0; i<stages.size(); i++)
            ((Stage)stages.get(i)).setLabelPosition(getBounds());
      }
   }

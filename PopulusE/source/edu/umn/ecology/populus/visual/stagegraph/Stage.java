...
//Color c = new Color(40,84,40);
   ...
   public void setPosition(Point p, Rectangle bounds){
      ...
      for(int i=0; i<starts.size(); i++)
         ((Transition)starts.get(i)).setStart(getCenter());
      for(int i=0; i<stops.size(); i++)
         ((Transition)stops.get(i)).setStop(getCenter());
   ...
   public void rotateLabel(Point p){
      ...
      double dtheta = arcTan(p.y-c.y, p.x-c.x) - arcTan((double)r.y,(double)r.x);//angle of drag relative to label

...
public class HMSSParamInfo implements BasicPlot {
   ...
   private double getLinkageDisequilibrium(int sex, int i){
      switch(i){
         case dAB: return (genotypicFrequencies[sex][A|B|C] + genotypicFrequencies[sex][A|C])*
         ...
         case dBC: return (genotypicFrequencies[sex][A|B|C] + genotypicFrequencies[sex][B|C])*
         ...
         case dAC: return (genotypicFrequencies[sex][A|B|C] + genotypicFrequencies[sex][A|B])*
      ...
      if(isMono){
         if(r > p)
            switch ( mate & (C | A<<maleBit) ){
               case /*CxA*/ C | A<<maleBit:   return p;
               case /*Cxa*/ C:                return (r - p);
               case /*cxA*/ A<<maleBit:       return 0.0;
               case /*cxa*/ 0:                return (1.0 - r);
            }
         else
            switch ( mate & (C | A<<maleBit) ){
               case /*CxA*/ C | A<<maleBit:   return r;
               case /*Cxa*/ C:                return 0.0;
               case /*cxA*/ A<<maleBit:       return (p - r);
               case /*cxa*/ 0:                return (1.0 - p);
            }
      } else {
         switch ( mate & (C | A<<maleBit) ){
            case /*CxA*/ C | A<<maleBit:      return r;
            case /*Cxa*/ C:                   return 0.0;
            case /*cxA*/ A<<maleBit:          return p*(1.0 - r);
            case /*cxa*/ 0:                   return (1.0 - p)*(1.0 - r);
         }
      }
      return -1;
   }
   ...
   double anderssonMatingFreq(double r, double pB, double pb, int mate){
      if(r > pB)
         switch ( mate & (C | (A|B)<<maleBit) ){//001 110
            case /*CxAB*/ C | (A|B)<<maleBit:   return pB;
            case /*CxAb*/ C | (A)<<maleBit:     return (r - pB)*pb/(1.0 - pB);
            case /*Cxab*/ C:
            case /*CxaB*/ C | (B)<<maleBit:     return (r - pB)*(1.0 - pB - pb)/(1.0 - pB);
            case /*cxAB*/ (A|B)<<maleBit:       return 0.0;
            case /*cxAb*/ A<<maleBit:           return (1.0 - r)*pb/(1.0 - pB);
            case /*cxab*/ 0:
            case /*cxaB*/ B<<maleBit:           return (1.0 - r)*(1.0 - pB - pb)/(1.0 - pB);
         }
      else
         switch ( mate & (C | (A|B)<<maleBit) ){//001 110
            case /*CxAB*/ C | (A|B)<<maleBit:   return r;
            case /*CxAb*/ C | (A)<<maleBit:     return 0.0;
            case /*Cxab*/ C:
            case /*CxaB*/ C | (B)<<maleBit:     return 0.0;
            case /*cxAB*/ (A|B)<<maleBit:       return (pB - r);
            case /*cxAb*/ A<<maleBit:           return pb;
            case /*cxab*/ 0:
            case /*cxaB*/ B<<maleBit:           return (1.0 - pB - pb);
         }
      return -1;
   }
   ...
   public BasicPlotInfo getBasicPlotInfo() {
      ...
      switch(plotType){
         case pqrvsT:
         case DvsT:
            points = new double[3][2][gens+1];
            break;
         case VvsT:
            points = new double[2][2][gens+1];
            break;
         case pqDAB:
         case rqDBC:
      ...
      for(int i=0; i<=gens; i++){
         ...
         genotypicFrequencies[male] = (double[])genotypicFrequencies[female].clone();
      ...
      switch(plotType){
         case pqrvsT:
            ycap = "Allelic Frequency (  "+ColorScheme.getColorString(0)+"<i><b>p</>, "+ColorScheme.getColorString(1)+"<i><b>q</>, "+ColorScheme.getColorString(2)+"<i><b>r</> )";
            break;
         case DvsT:
            ycap = "Disequilibria (  "+ColorScheme.getColorString(0)+"<b>D<sub>AB</>, "+ColorScheme.getColorString(1)+" <b>D<sub>BC</>, "+
                   ColorScheme.getColorString(2)+" <b>D<sub>AC</> )";
            break;
         case VvsT:
            ycap = ColorScheme.getColorString(0)+"Male</> and "+ColorScheme.getColorString(1)+"Female</> Viability ";
            break;
         case pqDAB:
            xcap = "<i>p</>";
            ycap = "<i>q</>";

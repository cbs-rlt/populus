	...
	class DrawingPane extends JPanel implements MouseListener, MouseMotionListener, ActionListener{
      ...
      actually added, they can be forgotten
		...
		Transition setUpTransition(int stageStart, int stageStop){
			...
			for(int i=0; i<trans.size(); i++){
				c = trans.get(i).getConnectionIndicies();
				if(c[0] == stageStop && c[1] == stageStart){
					t.setStraight(false);
					trans.get(i).setStraight(false);
				}
		...
		public void paint(Graphics g){
			...
			for(int i=0; i<trans.size(); i++){
				trans.get(i).setTimeIndex(timeIndex);
				trans.get(i).paint(g);
			}
			...
			for(int i=0; i<trans.size(); i++)
				trans.get(i).paintLabel(g);
			for(int i=0; i<stages.size(); i++)
				stages.get(i).paintLabel(g);
		...
		public StageShape getSelection(Point p){
			for(int i=0; i<trans.size(); i++)
				if(trans.get(i).getLabel().contains(p))
					return trans.get(i).getLabel();
			for(int i=0; i<stages.size(); i++)
				if(stages.get(i).getLabel().contains(p))
					return stages.get(i).getLabel();
			for(int i=0; i<stages.size(); i++)
				if(stages.get(i).contains(p))
					return stages.get(i);
			for(int i=0; i<trans.size(); i++)
				if(trans.get(i).contains(p))
					return trans.get(i);
			return null;
		}
		...
		public void mouseMoved(MouseEvent e) {
			...
			for(int i=0; i<stages.size(); i++){
				...
				stages.get(i).l.setActive(false);
			}
			for(int i=0; i<trans.size(); i++){
				trans.get(i).setActive(false);
				trans.get(i).l.setActive(false);
			}
		...
		void this_componentResized(ComponentEvent e) {
         ...
         default positions*/
			for(int i=0; i<stages.size(); i++)
				stages.get(i).setLabelPosition(getBounds());
		}
	}

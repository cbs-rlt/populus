...
Almost all of the code is in \texttt{PopPreferences.java}.
...
Note that there are distribution licenses associated with this.  University of Minnesota has a license to redistribute the libraries as part of Populus.

We could maybe replace this with JFreeChart some day, but haven't done that.  Although I've added a switch now in \texttt{PopPreferences}
...
Ideally, we'd also like to test that it will install a proper version of Java if it is not installed already.

Permissions are a bit trickier now, too, such as Gatekeeper...

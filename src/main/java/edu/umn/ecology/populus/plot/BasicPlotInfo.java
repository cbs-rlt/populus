        ...
        JCChartListener  //changeChart, paintChart
{
   ...
   regardless of what xMin, xMax have been set to*/
    ...
    private Stroke getLineStroke(int n) {
        ...
        return new BasicStroke(jccs.getLineWidth(),
    ...
    public void setData(double[][][] points) {
        data = null;
        for (double[][] point : points) addData(point);
    }
    ...
    public void findBounds() {
        ...
        for (double[][] doubles : data) {
            for (int j = 0; j < doubles[0].length; j++) {
                currentX = doubles[0][j];
                currentY = doubles[1][j];
        ...
        if (data[0].length == 3) {
            ...
            for (double[][] datum : data) {
                for (int j = 0; j < datum[2].length; j++) {
                    currentZ = datum[2][j];
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println(String.join(";", xCaption)
                + res.getString("vs_") + String.join(";", yCaption));
        e = lines.elements();
        while (e.hasMoreElements()) {
